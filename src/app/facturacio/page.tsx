...
import { nextFacturaNum } from "./actions";
...
export default async function FacturacioPage() {
  ...
  const [factures, suplits, conceptes, clients, expedients, invoiced, suplitSugg, suggestedNum] = await Promise.all([
    sql`
      select f.id, f.num, f.estat, to_char(f.data, 'YYYY-MM-DD') as data, f.client_id, c.nom as client_nom, c.nif,
             ...
             e.categoria as expedient_categoria, e.tipus as expedient_tipus, et.nom as expedient_tipologia,
             f.concepte, f.lang,
             f.preu::text as preu, coalesce(f.iva_pct, 21)::text as iva_pct, f.pagada,
      ...
      left join public.tipologies et on et.id = e.tipologia_id
    ...
    sql`select id, factura_id, descripcio, import::text as import, ordre from public.factura_concepte order by ordre, id` as unknown as Promise<FacturaConcepte[]>,
