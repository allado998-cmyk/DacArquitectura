...
export default async function FacturacioPage() {
  ...
  const [factures, suplits, clients, expedients, invoiced, suplitSugg] = await Promise.all([
    sql`
      select f.id, f.num, to_char(f.data, 'YYYY-MM-DD') as data, f.client_id, c.nom as client_nom, c.nif,
      ...
      order by f.num desc nulls last, f.id desc
